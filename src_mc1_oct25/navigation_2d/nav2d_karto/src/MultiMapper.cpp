...
MultiMapper::MultiMapper()
{
	...
	// For making a thread for scancb:
...
void MultiMapper::processLatestScan()
{
...

	{
		...
		if(mState == ST_MAPPING)
		{
			...
			if(success)
			{
				...
				// Send the scan to the other robots via com-layer (DDS)
				ROS_INFO("Robot %d: Sending scan (uniqueID: %d, Sensor: %s, stateID: %d)", mRobotID, laserScan->GetUniqueId(), laserScan->GetSensorIdentifier().ToString().ToCString(), laserScan->GetStateId());
				
				// when this func ends, boost will try to destroy this shared ptr as well as the object in it, i.e. latest_Scan_Recv.
				
				// Oct: converted sendLocalizedScan's input type.	
				// boost::shared_ptr<sensor_msgs::LaserScan> cps (&lsc);
	...
	if (scan_cb_times.size()%20 == 3)
	{
		...
		write_arr_to_file(tput_map_cb, ss);
	}

	// For converting navigation2d to ED:
	// Publishing transform from within scanCB
	// THis has to be at the end of processing the LaserScan so its published right after its updated in the func.
	publishTransform();
...
bool MultiMapper::updateMap()
{
	...
	ROS_ERROR("In MultiMapper::updateMap Current ratio of unknown/total area : %i %i #", unknown_area, total_area);
	...
	last_map_upd_out = now;

	if (map_update_ts.size() % 5 == 2)
	{
		// std::ofstream of;
		...
		write_arr_to_file(tput_map_update, mus);
		// of << "\n" << mus << " ScanCount: ";
