...
void RobotNavigator::receiveExploreGoal(const nav2d_navigator::ExploreGoal::ConstPtr &goal)
{
	...
	ROS_ERROR("IN NAV2D::NAVIGATOR, received StartExploration service action. recheckCycles %i, recheckThrottle %i, replanningPeriod %f", recheckCycles, recheckThrottle, mReplanningPeriod);
		...
		double using_map_ts, using_tf_scan_ts;
			...
			if(preparePlan())
			{
				...
				case EXPL_FINISHED:
					...
					ROS_ERROR("Exploration has finished. Total_Time_Taken_Clk: %f", (double)(explore_end_clk - explore_start_clk)/CLOCKS_PER_SEC);
			...
			if(mStatus == NAV_ST_EXPLORING)
			{
				if(success)
				{
					...
					// TODO: Do we measure tput only if it uses a new input? Would also need to check odometry TS.
				...
				}else
				{
					...
					explore_end_clk = clock();
					clock_gettime(CLOCK_MONOTONIC, &explore_end);
					ROS_ERROR("Exploration has failed. Total_Time_Taken_Clk: %f", (double)(explore_end_clk - explore_start_clk)/CLOCKS_PER_SEC);
			...
			double time_now = get_time_now();
			explore_cb_plan_ts.push_back(time_now);
...
void RobotNavigator::navGenerateCmdLoop()
{
	...
	while (nh.ok() && !nav_cmd_thread_shutdown_)
	{
		...
		if(mStatus == NAV_ST_EXPLORING)
		{
			...
			explore_cb_cmd_times.push_back(cmd_t);
			explore_cb_cmd_ts.push_back(time_now);
	
			// Measuring NavC tput : check if used a new tf or a new plan?
			// TODO: Measure tput always or only if new smt? Would also need to check for odometry TS.
