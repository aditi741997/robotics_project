...
for t in 20
do
    for freq in 15 #15 20 25 30 35 50 80 140 #12 28 44 60 80 100 140 
    do
	python change_obj_speed.py $t 1 #1 denotes walking, 2 denotes jumping [very fast walk]
        python change_cam_Freq.py $freq $bot
        source devel/setup.bash
        taskset -a -c 0-3 roslaunch turtlebot3_gazebo turtlebot3_empty_world.launch &
        sleep 15s
        ...
        taskset -a -c 6 rosrun beginner_tutorials subscriber 960000 1 $freq 8500 1 0 1 1 5500 listener chatter meh 1 > $prep_out 2> $prep_err &
        sleep 2s
        ...
        taskset -a -c 6 rosrun beginner_tutorials objdetector 1 1 8000 obj_detector 1 1 11000 > $vis_out 2> $vis_err &
        sleep 2s
        ...
        taskset -a -c 6 rosrun beginner_tutorials objtracker 3.5 > $track_out 2> $track_err &
        sleep 5s
        echo "Killing rostopic"
        kill -INT $(ps -ef | grep rostopic | grep -v grep | awk '{print $2}')
        perf_file="${x}_perf_$freq$t.out"
        echo $perf_file
        #eval "$(conda shell.bash hook)"
        #conda activate py2_opencv
        taskset -c 4 python src/rbx/src/measure_rbx_perf.py $perf_file &
        tim=$((8500/$freq))
        opt=25 #CHANGE THIS BASED ON ci !!!!!
        if [ $freq -lt $opt ]
        then
