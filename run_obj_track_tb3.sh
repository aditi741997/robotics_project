...
for r in 8 #9
do
    for freq in 15 #15 17 20 23 26 28 30 32 33 35 40 60 80 100 #12 28 44 60 80 100 140 
    do
	for t in 4
	do
	    python change_obj_speed.py $t 1 #1 denotes walking, 2 denotes jumping [very fast walk]
            ...
            cam_freq="${x}$r.g250_5cTD_Smallc1_actual_freq_$freq$t.out"
            echo "Starting rostopic"
            echo $cam_freq
            #taskset -a -c 5 rostopic hz /camera/rgb/image_raw > $cam_freq &
            sleep 10s
            sleep 2s
            prep_out="${x}$r.g250_5cTD_Smallc1_preprocess_node_$freq$t.out"
            prep_err="${x}$r.g250_5cTD_Smallc1_preprocess_node_$freq$t.err"
            lat="${x}$r.g250_5cTD_Smallc1_preprocess_lat_$freq$t.txt"
            echo $prep_out
            ...
            sleep 2s
            vis_out="${x}$r.g250_5cTD_Smallc1_vision_node_$freq$t.out"
            vis_err="${x}$r.g250_5cTD_Smallc1_vision_node_$freq$t.err"
            echo $vis_out
            ...
            sleep 2s
            track_out="${x}$r.g250_5cTD_Smallc1_tracker_node_$freq$t.out"
            track_err="${x}$r.g250_5cTD_Smallc1_tracker_node_$freq$t.err"
            echo $track_out
            # taskset -a -c 6 python src/rbx1/rbx1_apps/nodes/object_tracker2.py /camera/rgb 1 > $track_out 2> $track_err &
            taskset -a -c 6 rosrun beginner_tutorials objtracker 3.2 1 > $track_out 2> $track_err &
	    #taskset -a -c 5 rosrun beginner_tutorials controller 2 3 0.1 0.5 /camera/rgb/image_raw_cb_time /camera1/rgb/image_raw_cb_time /roi_cb_time & 
            sleep 5s
	    perf_file="${x}$r.g250_5cTD_Smallc1_perf_$freq$t.out"
            echo $perf_file
	    ...
	    taskset -c 5 python src/rbx/src/measure_rbx_perf.py $perf_file &
            ...
            then
                ...
                taskset -c 5 python measure_cpu_objtrack.py $t $freq $tim $x & 
	        sleep $tim
            else
                ti=500 #600
	        taskset -c 5 python measure_cpu_objtrack.py $t $freq $ti $x &
                sleep $ti
            fi
