...
Evaluation::evaluate(const std::vector<DAG>& dags)
{
	...
	for (const auto& eval : latencyEval_)
	{
		...
		std::cout << "Considering task chain " << std::endl;
		...
		for (unsigned k = 0; k < dags.size(); k++)
		{
			...
			std::cout << dags[k].getNodeInfo() << ", latency info DA : " << info.maxLatency << ", Lat(reactiontm) : " << info.reactionTime << std::endl;

			if (!eval.second.second.isValid(info))
			{
				cost[k] = NAN;
				invalidDags++;
				std::cout << "INVALID!" << invalidDags << std::endl;
				continue;
			}
