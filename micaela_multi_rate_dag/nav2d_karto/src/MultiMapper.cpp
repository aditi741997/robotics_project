...
MultiMapper::MultiMapper()
{
	...
	mMapper = new karto::OpenMapper(false); // Disabling tbb for now.
	...
	ROS_ERROR("Mapper: Have started mapCB thread, will now start mapUpd thread.");

        map_upd_exec_info_pub = robotNode.advertise<std_msgs::Header>("exec_start_mapupd", 1, true);

	ROS_ERROR("Mapper: made publisher for topic exec_start_mapupd, will now start mapUpd thread:");
...
void MultiMapper::mapUpdateLoop(double map_update_rate)
{
	...
	// Setting SCHED_DEADLINE policy:
        ...
        ROS_ERROR("Output of setting DDL policy for Mapper's MapUpdate Loop %i", oout);
        if (oout < 0)
                std::cerr << "setting DDL policy for Mapper's MapUpdate thread retVal=-1, error: " << std::strerror(errno) << std::endl;
        ...
        while (nh.ok() && !map_update_thread_shutdown_)
        {
                ...
                {
			/* Commenting cuz using deadline policy
...
void write_arr_to_file(std::vector<double>& tput, std::string s, std::string m)
{
    ...
    nh.param<std::string>("/expt_name", ename, "");
        ...
        if (sz > 0)
        {
                ...
                ROS_ERROR("Nav2d Mapper %s, Median, 95ile %s %f %f", s.c_str(), m, tput[sz/2], tput[(95*sz)/100]);
                tput.clear();
        }
...
void MultiMapper::mapScanCBLoop(double per)
{
	...
	attr.sched_deadline = period*1000*1000;
	int oout = sched_setattr(0, &attr, 0);
	ROS_ERROR("Output of setting DDL policy for Mapper's MapScanCB Loop %i", oout);
	if (oout < 0)
		std::cerr << "setting DDL policy for Mapper's MapScanCB thread retVal=-1, error: " << std::strerror(errno) << std::endl;
