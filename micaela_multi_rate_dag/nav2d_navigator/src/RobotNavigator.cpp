...
{	
	...
	try
	{
		...
		ROS_ERROR("Successfully loaded exploration strategy [%s].", mExplorationStrategy.c_str());
...
{	
	...
	{
		ROS_ERROR("IN ROBOTNavigator:: createPlan Goal coordinates in map: %i %i  #", goal_x, goal_y);
	...
	// Making new ptr, will then copy to class var. Doing this cuz Plan used by 2 threads.
...
bool RobotNavigator::generateCommand()
{
	...
	// make a copy of currentPlan here
	...
	// For measuring RT:
        ...
        msg.LastScanTSScanMapCBNavCmd = current_mapCB_tf_navCmd_scan_ts;
...
void RobotNavigator::receiveExploreGoal(const nav2d_navigator::ExploreGoal::ConstPtr &goal)
{
	...
	int oout = sched_setattr(0, &attr, 0);
        ROS_ERROR("Output of setting DDL policy for Navigator planner thread  %i", oout);
        if (oout < 0)
                std::cerr << "setting DDL policy for Navigator planner thread retVal=-1, error: " << std::strerror(errno) << std::endl;
...
void RobotNavigator::navGenerateCmdLoop()
{
	...
	// Setting SCHED_DEADLINE policy:
        ...
        attr.sched_deadline = period*1000*1000;
        int oout = sched_setattr(0, &attr, 0);
        ROS_ERROR("Output of setting DDL policy for Navigator Command thread %i", oout);
        if (oout < 0)
                std::cerr << "setting DDL policy for Navigator Command thread retVal=-1, error: " << std::strerror(errno) << std::endl;
...
bool RobotNavigator::setCurrentPosition(int x)
{
	...
	try
	{
		...
		if (x == 0)
                        current_mapCB_tf_navCmd_scan_ts = current_mapper_tf_scan_ts; // TS of the scan used by the mapper_TF used by the NavCmd.
                else if (x == 1)
                        latest_mapCB_tf_navPlan_scan_ts = current_mapper_tf_scan_ts; // TS of the scan used by the mapper_TF used by the NavPlan.
