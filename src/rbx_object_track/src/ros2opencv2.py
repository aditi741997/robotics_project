...
class ROS2OpenCV2(object):
    ...
    def image_callback(self, data):
        ...
        # rospy.loginfo("Done with putText " + self.node_name + ", " + str(self.display_image[0][0]))
        # Update the image display
        # plt.imshow(cv2.cvtColor(self.display_image, cv2.COLOR_BGR2RGB))
        # plt.show()
        # plt.close()
        # cv2.imshow(self.node_name, self.display_image)
        # rospy.loginfo("ABout to wait for key")
    ...
    def publish_roi(self):
        if not self.drag_start:
            if self.track_box is not None:
                # rospy.loginfo("Using trackbox")
                roi_box = self.track_box
            elif self.detect_box is not None:
                # rospy.loginfo("Using detect_box")
                roi_box = self.detect_box
            else:
                roi_box = (0,0,0,0)
                # Changing this since we always wanna publish!
        try:
        ...
        try:
            ...
            # rospy.loginfo("ABout to publish ROI!")
            roi_time_obj = roi_time()
            roi_time_obj.x_offset = int(roi_box[0])
            roi_time_obj.y_offset = int(roi_box[1])
            roi_time_obj.width = int(roi_box[2])
            roi_time_obj.height = int(roi_box[3])
            roi_time_obj.stamp = self.image_header.stamp
            # self.roi_pub.publish(ROI)
            self.roi_time_pub.publish(roi_time_obj)
        except:
