...
void YoloObjectDetector::cameraCallback(const sensor_msgs::ImageConstPtr& msg) {
  ...
  if (cam_yolo_count < 10)
  {
	  ...
	  yolo_thr_info_pub.publish(hdr);
  }

  double using_cam_ts = msg->header.stamp.toSec();
  printf("YOLO cam_yolo_count: %i ts: %f, frame id: %s", cam_yolo_count, using_cam_ts, msg->header.frame_id.c_str());

  cv_bridge::CvImagePtr cam_image;

  try {
	// loading the img here:
	cv::Mat img = cv::imread("/home/ubuntu/catkin_ws/NewJPGImgs/"+msg->header.frame_id+".jpg", CV_LOAD_IMAGE_COLOR);
	sensor_msgs::ImagePtr img_msg = cv_bridge::CvImage(std_msgs::Header(), "bgr8", img).toImageMsg();
	// didnt work: cam_image = boost::make_shared (cv_bridge::CvImage(std_msgs::Header(), "bgr8", img) );
  ...
  if (cam_image) {
    ...
    {
	    std::cout << "exception in letterbox_image_into: " << e << std::endl;
	    return;	   
    }
    ...
    float* X = buffLetter_[(buffIndex_) % 3].data;
    ...
    if (enableConsoleOutput_) {
      ...
      printf("\033[1;1H");
      printf("\nFPS:%.1f\n", fps_);
      printf("For image: %s Objects:\n\n", msg->header.frame_id.c_str());
    }
    image display = buff_[(buffIndex_) % 3];
    ...
    {
	    lat_cam_yolo_arr.push_back(lat_ppcam_yolo);
	    ...
	    if (cam_yolo_count > 1)
		    rt_cam_yolo_arr.push_back(time_now - last_cam_ts);

	    last_cam_ts = using_cam_ts;
	    last_cam_yolo_out = time_now;
    }

    std_msgs::Header y_exec_time;
    y_exec_time.frame_id = std::to_string( cpu_ci ) + " yolo";
    yolo_exec_end_pub.publish(y_exec_time);
  }
  return;
...
void* YoloObjectDetector::displayInThread(void* ptr) {
  ...
  show_image_cv(buff_[(buffIndex_) % 3], "YOLO V3", ipl_);
